...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="text" name="xingming" class="form-control" id="txt_search_username" >
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">角色</span>
					<select class="form-control" name="banjiid" id = "txt_search_roleid">
						<option value="0">---请选择---</option>
						<c:forEach items="${banjiList }" var="r">
						 	<option value="${r.banjiid }">${r.banji }</option>
						</c:forEach>
                	</select>
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm" enctype="multipart/form-data">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>姓名：</td>
								<td><input type="text" id="username" name="xingming"
							...
							<tr>
								<td>班级：</td>
								<td colspan="4">
									<select class="form-control" name="banjiid" id = "banjiid" aria-required="true" required>
							...
							<tr>
								<td>照片</td>
								<td>
									<input type="hidden" id="zhaopian">
									<img id="zhaopian1" height="70" width="100">
							...
							<tr>
								<td>生日</td>
								<td>
									<input type="date" name="shengri" id="shengri"> 
								</td>
							</tr>
							<tr>
								<td>爱好</td>
								<td>
									<input type="checkbox" name="hobby" value="小舞">小舞 
									<input type="checkbox" name="hobby" value="火无双">火无双 
									<input type="checkbox" name="hobby" value="洛水寒">洛水寒
								</td>
							</tr>
	...
	<!-- 导入 -->
	<div class="modal fade" id="modal_user_edit_daoru" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user_daoru" method="post" action="daoru.htm" enctype="multipart/form-data">
						<input type="file" name="tu">
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_user_btn_daoru">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	<!-- 柱状图 饼状图 -->
	<div class="modal fade" id="modal_user_edit_zhuzhuang" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<div id="zhuzhuangid" style="width:400px;height:400px;"></div>
   ...
   <script src="${path }/resources/js/plugins/layer/laydate/laydate.js"></script>
	...
	$(function () {
	    ...
	    $("#submit_form_user_btn_daoru").click(function(){
	    	$("#form_user_daoru").submit();
	    });
	    var validator = $("#form_user_daoru").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
   		    		...
   		    		if(data.success ){
   		    			validator.resetForm();
   		    			$('#modal_user_edit_daoru').modal('hide');
   		    			$("#btn_search").click();
   		    		}else{
   		    			$("#select_message").text(data.errorMsg);
   		    			$("#alertmod_table_user_mod").show();
	...
	var TableInit = function () {
	    ...
	    oTableInit.Init = function () {
	        $('#table_user').bootstrapTable({
	           ...
	           // height: 500,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	            uniqueId: "bianhao",                     //每一行的唯一标识，一般为主键列
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            xingming: $("#txt_search_username").val(),
	            banjiid: $("#txt_search_roleid").val(),
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	//柱状
	        $("#btn_zhuzhuang").click(function(){
	    		/* $('#password').attr("readOnly",false).val(getSelection.password);
	    		$("#form_user").resetForm();
	    		document.getElementById("hidden_txt_userid").value='';
	    		$('#modal_user_edit_zhuzhuang').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_edit_zhuzhuang').modal('show'); */
				 $('#modal_user_edit_zhuzhuang').modal({backdrop: 'static', keyboard: false});
		         $('#modal_user_edit_zhuzhuang').modal('show');
				$.ajax({
	    		    url:"zhuzhuang.htm",
	    		    ...
	    		    success:function(res){
	    		    	...
	    		    	alert(h)
	    		    	 ...
	    		    	 var myChart = echarts.init(document.getElementById('zhuzhuangid'));
	    		         // 指定图表的配置项和数据
	    		         var option = {
	    		 		 title: {
	    		                 text: '班级人数'
				...
				$.ajax({
	    		    ...
	    		    success:function(res){
	    		    	...
	    		    	for (var i = 0; i < jihe.length; i++) {
	    		    		//j[i]=jihe[i].banji;
	    		    		j[i]=jihe[i].banji;
	    		    		h[i]={value:jihe[i].renshu,name:jihe[i].banji};
						}
	    		    	alert(j)
	    		    	alert(h)
   ...
   // 使用刚指定的配置项和数据显示图表。
        myChart.setOption(option);
	    	...
	    	$("#btn_daoru").click(function(){
	    		...
	    		$("#form_user").resetForm();
	    		document.getElementById("hidden_txt_userid").value='';
	    		$('#modal_user_edit_daoru').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_edit_daoru').modal('show');
	        });
	...
	function initEditUser(getSelection){
		...
		$('#bianhao').val(getSelection.bianhao)
		$('#zhaopian').val(getSelection.zhaopian)
		$('#zhaopian1').attr("src",getSelection.zhaopian)
		
			$("input[value='"+getSelection.xingbie+"']").prop("checked",true)
			
		$('#shengri').val(new Date(getSelection.shengri).Format('yyyy-MM-dd'))
		
		$('#banjiid').val(getSelection.banjiid);
		$('#username').val(getSelection.xingming);
		$('#userdescription').val(getSelection.userdescription);
		//$('#password').attr("readOnly",true).val(getSelection.password);
		
		if(getSelection.aihao!=null){
			var hobby=getSelection.aihao;
			
			var h=hobby.split(" ")
			
			for (var i = 0; i < h.length; i++) {
				
				$('input[value="'+h[i]+'"]').prop("checked",true)
