...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>入职时间：</td>
								<td>
									<input type="date" name="shijian" id="shijian">
									</td>
	...
	<div class="modal fade" id="modal_user_edit_lianxi" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user_lianxi" method="post" action="lianxi.htm">
						<input type="hidden" name="id" id="hidden_txt_userid" value=""/>
						<!-- <input type="hidden" name="id" id="id" value=""> -->
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_user_btn_lianxi">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    var validator = $("#form_user_lianxi").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
   		    		...
   		    		if(data.success && !data.errorMsg ){
   		    			...
   		    			$('#modal_user_edit_lianxi').modal('hide');
   		    			$("#btn_search").click();
   		    		}else{
   		    			$("#select_message").text(data.errorMsg);
   		    			$("#alertmod_table_user_mod").show();
	    ...
	    $("#submit_form_user_btn_lianxi").click(function(){
	    	$("#form_user_lianxi").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_lianxi").click(function(){
	    		$('#password').attr("readOnly",false).val(getSelection.password);
	    		$("#form_user").resetForm();
	    		document.getElementById("hidden_txt_userid").value='';
	    		$('#modal_user_edit_lianxi').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_edit_lianxi').modal('show');
	        });
	...
	function initEditUser(getSelection){
		...
		$('#shijian').val(new Date(getSelection.shijian).Format('yyyy-MM-dd'))
		$('#pname').val(getSelection.pname);
